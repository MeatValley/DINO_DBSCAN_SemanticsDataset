...
def save_point_cloud_with_labels(points, labels, config, K, n_img):
    ...
    final_path = os.path.join(path, 'image2_robm.ply')
    print(final_path)
...
def show_point_cloud_spectral_clustering(point_label_corresp, pc_points):
    """
    point_label_corresp is a dict point:label_color
    """
    print('[creating the point cloud for spectral clustering...]')
    # print(point_label_corresp)
    points = []
    colors = []
    for j, point in enumerate(pc_points):
        #will pass every point
        if j in point_label_corresp.keys():
            point = np.asarray(point)
            color = FIX_COLORS[point_label_corresp[j]]
...
def get_point_clouds_with_labels(point_clouds_np, labels, random_colors = False):
    print('[getting point cloud with labels...]')
    """ Show a list of point clouds with their labels.
...
def get_point_cloud_spectral_clustering(point_label_corresp, pc_points):
    """
    point_label_corresp is a dict point:label_color
    """
    print('[creating the point cloud for spectral clustering...]')
...
def reduce_point_cloud(point_cloud, max_points = 0, max_fraction = 0.2):
    ...
    if max_points == 0: max_points = int(N*max_fraction)
    if N > max_points:
